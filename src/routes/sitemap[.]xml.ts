...
// Top capitals + major Brazilian cities for SEO city pages.
const CITIES = [
  ...
  "vila-velha","praia-grande","novo-hamburgo","ribeirao-preto","sao-jose-do-rio-preto",
];
  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        const urls: string[] = [];
        const push = (loc: string, lastmod?: string, changefreq = "weekly", priority = "0.7") => {
          urls.push(
        ...
        for (const c of CITIES) push(`/entrega/${c}`, today, "weekly", "0.8");
